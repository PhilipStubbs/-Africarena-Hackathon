<?php
	require('Users/connect_database.php');
	session_start();
	...
	if (isset($_POST['reg_user']))
	{
		...
		if (count($errors) == 0)
		{
			...
			$insert = "INSERT INTO $dbname.users (username, firstname, surname, email, password, confirmcode, confirmed) 
						VALUES('$username', '$firstname', '$surname', '$email', '$password', '$confirmcode' ,$bool)";
			$conn->exec($insert);
	...
	if (isset($_POST['reg_driver']))
	{
		...
		$usercheck = $conn->prepare("SELECT * FROM $dbname.drivers WHERE username = :usr");
		$usercheck->execute(["usr"=>$username]);
		$result_1 = $usercheck->fetchAll();

		$emailcheck = $conn->prepare("SELECT * FROM $dbname.drivers WHERE email = :eml");
		$emailcheck->execute(["eml"=>$email]);
		$result_2 = $emailcheck->fetchAll();
		...
		if (empty($vehicle_type))
			array_push($errors, "vehicle type is required");
		if (empty($vehicle_reg))
			array_push($errors, "vehicle reg is required");
		if (empty($capacity))
		...
		if (count($errors) == 0)
		{
			...
			$confirmcode = hash("ripemd160", $username);
			$insert = "INSERT INTO $dbname.drivers (username, firstname, surname, email, password, confirmcode, vehicle, reg, cap) 
						VALUES('$username', '$firstname', '$surname', '$email', '$password', '$confirmcode', '$vehicle_type', '$vehicle_reg', '$capacity')";
			$conn->exec($insert);
